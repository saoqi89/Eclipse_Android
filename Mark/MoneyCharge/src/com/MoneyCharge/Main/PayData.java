...
import com.MoneyCharge.Model.KindData;
	...
	public PayData() {
	 ...
	 @Override
	    public void onCreate(Bundle savedInstanceState) {
	        ...
	        adapter=new ArrayAdapter<String>(PayData.this,android.R.layout.simple_spinner_item,yearlist);
	 ...
	 protected void onStart(){
	        ...
	        mRenderer.setLabelsColor(Color.BLACK);
		 	...
		 	mRenderer.setChartTitleTextSize(54);// 设置饼图标题大小
		 	 ...
		 	 if(type==0){ 
		 		KindDatap=payDAO.getKDataOnMonth(userid, defaultYear, defaultMonth); 	
		 	...
		 	//数据
		 	 ...
		 	 if(KindDatap.size()==0){ 
		 		...
		 		for(KindData kp:KindDatap){ 
		 			mSeries.add(ptypeDAO.getOneName(userid, kp.getKindname()), kp.getAmount()/ sum);
	 ...
	 private int getRandomColor() {// 分别产生RBG数值  
		...
		public boolean onKeyDown(int keyCode, KeyEvent event) {
		    if(keyCode == KeyEvent.KEYCODE_BACK) { //监控/拦截/屏蔽返回键
		    	Intent intent=new Intent(PayData.this,Data.class);
