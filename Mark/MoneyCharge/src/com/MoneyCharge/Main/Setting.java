...
import com.MoneyCharge.DAO.NoteDAO;
import com.MoneyCharge.DAO.PayDAO;
...
public class Setting extends Activity{
	...
	private void alarmDialog(int type){  //退出程序的方法 
		 ...
		 switch(type){
		 	case 0:
				 customBuilder.setMessage("将删除当前的用户所有"+is).setPositiveButton("确定", new DialogInterface.OnClickListener() {
	
			           public void onClick(DialogInterface dialog, int which) { 
			        	   	IncomeDAO incomeDAO=new IncomeDAO(Setting.this);
			        	   	incomeDAO.deleteUserData(userid); 
							Toast.makeText(Setting.this,"已清空~！！",Toast.LENGTH_LONG).show();
			            }
		 	...
		 	case 1:
				 customBuilder.setMessage("将删除当前的用户所有"+ps).setPositiveButton("确定", new DialogInterface.OnClickListener() {
		 	...
		 	case 2:
				 ...
				 break;
		 	case 5:
				 customBuilder.setMessage("此操作将重置当前用户的收入、支出类型，确定还原吗？").setPositiveButton("确定", new DialogInterface.OnClickListener() {
			           ...
			           public void onClick(DialogInterface dialog, int which) { 
			        	   ItypeDAO itypedao=new ItypeDAO(Setting.this);
							...
							Toast.makeText(Setting.this,"已还原~！！",Toast.LENGTH_LONG).show();
							 dialog.dismiss();
			            }
