...
public int add(TB_Account tb_account) {
	...
	if(tb_account.get_id()==0){
		db.execSQL("insert into tb_account (username,pwd) values (?,?)",
				new Object[] { tb_account.getUsername(),tb_account.getPwd() });
	}
	else{
	db.execSQL("insert into tb_account (_id,username,pwd) values (?,?,?)",
			new Object[] { tb_account.get_id(),tb_account.getUsername(),tb_account.getPwd() });
	}
	 Cursor cursor = db.rawQuery("select _id from tb_account where username=? and pwd=?",new String[]{tb_account.getUsername(),tb_account.getPwd()});
...
public TB_Account find(String username,String pwd) {
	db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
	// 查找密码并存储到Cursor类中
	Cursor cursor = db.rawQuery("select _id, username, pwd from tb_account where pwd=? and username=? ",new String []{pwd,username} );
	if (cursor.moveToNext())// 遍历查找到的密码信息
	{
		// 将密码存储到Tb_account类中
...
public String find(int id) {
	db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
	// 查找密码并存储到Cursor类中
	Cursor cursor = db.rawQuery("select username from tb_account where _id=? ",new String []{String.valueOf(id)} );
	if (cursor.moveToNext())// 遍历查找到的密码信息
	{
		// 将密码存储到Tb_account类中
