...
public class PayDAO {
	...
	public void add(TB_Pay tb_pay) {
		...
		// 执行添加支出信息操作
		db.execSQL(
				"insert into tb_pay (_id,no,money,time,type,address,mark) values (?,?,?,?,?,?,?)",
				new Object[] { tb_pay.get_id(),tb_pay.getNo(), tb_pay.getMoney(),
						tb_pay.getTime(), tb_pay.getType(),
	...
	public void update(TB_Pay tb_pay) {
		...
		// 执行修改支出信息操作
		db.execSQL(
				"update tb_pay set money = ?,time = ?,type = ?,address = ?,mark = ? where _id = ? and no=?",
				new Object[] { tb_pay.getMoney(),
						tb_pay.getTime(), tb_pay.getType(),
						tb_pay.getAddress(), tb_pay.getMark(),
						tb_pay.get_id(),tb_pay.getNo() });
	}
	...
	public List<TB_Pay> getScrollData(int id,int start, int count) {
		...
		db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
		// 获取所有支出信息
		Cursor cursor = db.rawQuery("select _id,no,money,time,type,address,mark from tb_pay where _id=? order by no limit ?,?",
				new String[] { String.valueOf(id), String.valueOf(start), String.valueOf(count) });
		while (cursor.moveToNext())// 遍历所有的支出信息
		{
			// 将遍历到的支出信息添加到集合中
			tb_pay.add(new TB_Pay(
					cursor.getInt(cursor.getColumnIndex("_id")),
					cursor.getInt(cursor.getColumnIndex("no")),
					cursor.getDouble(cursor.getColumnIndex("money")),
					cursor.getString(cursor.getColumnIndex("time")), 
	...
	public long getCount() {
		db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
		Cursor cursor = db.rawQuery("select count(no) from tb_pay",
				null);// 获取支出信息的记录数
	...
	public long getCount(int id) {
		db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
		Cursor cursor = db
				.rawQuery("select count(no) from tb_pay where _id=?", new String[]{String.valueOf(id)});// 获取收入信息的记录数
		if (cursor.moveToNext())// 判断Cursor中是否有数据
		{
			return cursor.getLong(0);// 返回总记录数
		}
	...
	public int getMaxNo(int id) {
		db = helper.getWritableDatabase();// 初始化SQLiteDatabase对象
		Cursor cursor = db.rawQuery("select max(no) from tb_pay where _id=?",new String[]{String.valueOf(id)} );// 获取支出信息表中的最大编号
		...
		public String addDays(String sdate, int days)
	    {
			...
			Cursor cursor = db.rawQuery("select total(money) as tmoney from tb_pay  where time >= ? and time <= ? and  _id =?",new String[]{date1,date2,String.valueOf(userid)}  );// 获取支出信息表中的最大编号
			while (cursor.moveToNext())// 遍历所有的支出信息
			{		
			...
			public List<KindData> getKDataOnDay(int id,String date1,String date2){
				...
				Cursor cursor = db.rawQuery("select type,  total(money) as tmoney from tb_pay  where time >= ? and time <= ? and  _id =? group by type",new String[]{date1,date2,String.valueOf(id)} );// 获取支出信息表中的最大编号
				while (cursor.moveToNext())// 遍历所有的支出信息
				{		
